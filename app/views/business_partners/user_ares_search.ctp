	...
	<tr>
		...
		<td><?php
			...
			if (count($address) == 3) {
				...
				$street_no = $street[count($street) - 1];
				unset($street[count($street) - 1]);
				$street_name = trim(implode(' ', $street));
				echo $form->hidden('Address.0.city', array('value' => $city));
				echo $form->hidden('Address.0.number', array('value' => $street_no));
				echo $form->hidden('Address.0.street', array('value' => $street_name));
				...
				unset($street[count($street) - 1]);
				echo $form->hidden('Address.0.street', array('value' => trim(implode(' ', $street))));
