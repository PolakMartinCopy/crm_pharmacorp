...
class ImpositionsController extends AppController {
	...
	function user_index() {
		...
		$this->set('export_fields', $this->Imposition->export_fields);
		...
		$joins = array(
			...
			array(
				...
				'alias' => 'Purchaser',
				'type' => 'INNER',
				'conditions' => array('Imposition.purchaser_id = Purchaser.id')
			),
			array(
				'table' => 'users',
				'alias' => 'User',
		...
		$fields = array(
			...
			'Imposition.id', 'Imposition.description', 'Imposition.title', 'Solution.purchaser_name',
		...
		$this->Imposition->Solution->virtualFields['purchaser_name'] = $this->Imposition->Purchaser->virtualFields['name'];
		$assigned_solutions = $this->Imposition->Solution->find('all', $assigned_solutions_find);
		unset($this->Imposition->Solution->virtualFields['purchaser_name']);
	...
	function user_add() {
		...
		$autocomplete_purchasers = $this->Imposition->Purchaser->autocomplete_list($this->user);
		$this->set('purchasers', $autocomplete_purchasers);
		...
		if (isset($this->data)) {
			...
			if (empty($this->data['Imposition']['purchaser_id']) && !empty($this->data['Imposition']['purchaser_id_old'])) {
				$this->data['Imposition']['purchaser_id'] = $this->data['Imposition']['purchaser_id_old'];
			}
	...
	function user_edit($id = null) {
		...
		$imposition = $this->Imposition->find('first', array(
			...
			'contain' => array(
				...
				'Purchaser' => array(
					'Address' => array(
						'conditions' => array('Address.address_type_id' => 4)
					)
				),
				'RecursiveImposition',
				'Solution' => array(
		...
		if (isset($this->data)) {
			...
			if (empty($this->data['Imposition']['business_partner_name'])) {
				$this->data['Imposition']['purchaser_id'] = '';
				$this->data['Imposition']['purchaser_id_old'] = '';
			}
			if (empty($this->data['Imposition']['purchaser_id']) && !empty($this->data['Imposition']['purchaser_id_old'])) {
				$this->data['Imposition']['purchaser_id'] = $this->data['Imposition']['purchaser_id_old'];
			}
			...
			$this->data['Imposition']['purchaser_name'] = $this->Imposition->Purchaser->autocomplete_field_info($imposition['Purchaser']['id']);
