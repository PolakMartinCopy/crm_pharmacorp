...
class AnniversariesController extends AppController {
	...
	function user_index() {
		...
		$purchaser_conditions = array(
			'Purchaser.id = ContactPerson.purchaser_id'
		);
		
		if ($user['User']['user_type_id'] == 3) {
			$purchaser_conditions[] = 'Purchaser.user_id = ' . $user_id;
		}
		...
		$today_anniversaries = $this->Anniversary->find('all', array(
			'fields' => array('Anniversary.*', 'ContactPerson.*', 'Purchaser.*', 'AnniversaryType.*', 'AnniversaryAction.*', 'STR_TO_DATE( CONCAT_WS( \',\', Month( Anniversary.date ) , Day( Anniversary.date ) , Year( CURDATE( ) ) ) , \'%m,%d,%Y\' ) AS univ_date'),
			'conditions' => array(
				'OR' => array(
			...
			'joins' => array(
				...
				array(
					...
					'alias' => 'Purchaser',
					'type' => 'INNER',
					'conditions' => $purchaser_conditions
				)
			)
		));
		...
		$interval_anniversaries = $this->Anniversary->find('all', array(
			'fields' => array('Anniversary.*', 'ContactPerson.*', 'Purchaser.*', 'AnniversaryType.*', 'AnniversaryAction.*'),
			'conditions' => array(
				'OR' => array(
		...
		if (!empty($purchaser_conditions)) {
			$this->paginate['Anniversary']['conditions'] = $purchaser_conditions;
		}
