...
<h1>Upravit obchodní jednání</h1>
<ul>
...
<?php echo $form->create('BusinessSession', array('url' => array('controller' => 'business_sessions', 'action' => 'edit') + $this->passedArgs))?>
<table class="left_heading">
	...
	<tr>
		<th>Edukace</th>
		<td colspan="5"><?php echo $this->Form->input('BusinessSession.is_education', array('label' => false, 'type' => 'checkbox', 'checked' => (isset($this->data['BusinessSession']['is_education']) ? $this->data['BusinessSession']['is_education'] : false)))?></td>
	</tr>
	...
	<tr class="education" data-education-count="<?php echo $i?>">
		<td colspan="6"><?php
			echo $i+1 ?>. edukace
		<?php 
			echo $this->Form->hidden('Contract.' . $i . '.id');
			echo $this->Form->hidden('Contract.' . $i . '.confirmed', array('value' => false));
			echo $this->Form->hidden('Contract.' . $i . '.confirm_requirement', array('value' => false));
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
	...
	<?php foreach ($this->data['Contract'] as $i => $data) {
			// je dohoda editovatelna? (neni, pokud byla schvalena nebo odeslana ke schvaleni)
			$is_editable = (!(isset($data['confirmed']) && $data['confirmed']) && !(isset($data['confirm_requirement']) && $data['confirm_requirement']));
	?>
	<tr class="education" data-education-count="<?php echo $i?>">
		<td colspan="6"><?php
			echo $i+1 ?>. edukace
		<?php 
			echo $this->Form->hidden('Contract.' . $i . '.id');
			echo $this->Form->hidden('Contract.' . $i . '.confirmed', array('value' => (isset($data['confirmed']) ? $data['confirmed'] : false)));
			echo $this->Form->hidden('Contract.' . $i . '.confirm_requirement', array('value' => (isset($data['confirm_requirement']) ? $data['confirm_requirement'] : false)));
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
		<th style="width:100px">Kontaktní osoba</th>
		<td colspan="4"><?php
			// pokud muzu dohodu editovat, zobrazim normalni input
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.contact_person_name', array('label' => false, 'size' => 50, 'class' => 'contact-person-name'));
			// jinak zobrazim "nahrazku", ktera je disabled a hodnotu predam v hidden poli				
			} else {
				echo $this->Form->input('Contract.' . $i . '.contact_person_name_dis', array('label' => false, 'size' => 50, 'disabled' => true, 'value' => $data['contact_person_name']));
				echo $this->Form->hidden('Contract.' . $i . '.contact_person_name');
			}
			echo $this->Form->error('Contract.' . $i . '.contact_person_id');
			echo $this->Form->error('Contract.' . $i . '.number');
			echo $this->Form->error('Contract.' . $i . '.city');
	...
	<tr class="education" data-education-count="<?php echo $i?>">
		...
		<td colspan="4"><?php
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.contract_type_id', array('label' => false, 'options' => $contract_types, 'empty' => false));
			} else {
				echo $this->Form->input('Contract.' . $i . '.contract_type_id_dis', array('label' => false, 'options' => $contract_types, 'empty' => false, 'selected' => $data['contract_type_id'], 'disabled' => true));
				echo $this->Form->hidden('Contract.' . $i . '.contract_type_id');
			}
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
		...
		<td colspan="2"><?php
			if ($is_editable) {
				...
				echo $this->Form->input('Contract.' . $i . '.bank_account_dis', array('label' => false, 'disabled' => true, 'value' => $data['bank_account']));
				echo $this->Form->hidden('Contract.' . $i . '.bank_account');
			}
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
		...
		<td><?php
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.begin_date', array('label' => false, 'type' => 'text', 'class' => 'contract-date'));
			} else {
				echo $this->Form->input('Contract.' . $i . '.begin_date_dis', array('label' => false, 'type' => 'text', 'disabled' => true, 'value' => $data['begin_date']));
				echo $this->Form->hidden('Contract.' . $i . '.begin_date');
			}
		?></td>
		<th>Datum ukončení</th>
		<td colspan="2"><?php
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.end_date', array('label' => false, 'type' => 'text', 'class' => 'contract-date'));
			} else {
				echo $this->Form->input('Contract.' . $i . '.end_date_dis', array('label' => false, 'type' => 'text', 'disabled' => true, 'value' => $data['end_date']));
				echo $this->Form->hidden('Contract.' . $i . '.end_date');
			}
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
		...
		<td><?php
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.month', array('label' => false, 'options' => $months));
			} else {
				echo $this->Form->input('Contract.' . $i . '.month_dis', array('label' => false, 'options' => $months, 'disabled' => true, 'selected' => $data['month']));
				echo $this->Form->hidden('Contract.' . $i . '.month');
			}
		?></td>
		<th>Rok</th>
		<td colspan="2"><?php
			if ($is_editable) {
				echo $this->Form->input('Contract.' . $i . '.year', array('label' => false, 'size' => 5));
			} else {
				echo $this->Form->input('Contract.' . $i . '.year_dis', array('label' => false, 'size' => 5, 'disabled' => true, 'value' => $data['year']));
				echo $this->Form->hidden('Contract.' . $i . '.year');
			}
		?></td>
	</tr>
	<tr class="education" data-education-count="<?php echo $i?>">
