...
$tcpdf->Cell(90, 0, $purchaser['Purchaser']['name'], 0, 1, 'L', false);

$street_info = '';
$city_info = '';
if (!empty($purchaser['Address'])) {
	$street_info = $purchaser['Address']['street'] . ' ' . $purchaser['Address']['number'];
	if (!empty($purchaser['Address']['o_number'])) {
		$street_info .= '/' . $purchaser['Address']['o_number'];
	}
	$city_info = $purchaser['Address']['zip'] . ' ' . $purchaser['Address']['city'];
}
$ico_info = '';
if (!empty($purchaser['Purchaser']['ico'])) {
	$ico_info = 'IČO: ' . $purchaser['Purchaser']['ico'];
}
